...
class Snake:
	...
	def update_move_modif(self, new_modif):
		if new_modif == self.M_LEFT and self.head_orientation != self.M_RIGHT:
			self.move_modif = -1
			self.head_orientation = self.M_LEFT
		elif new_modif == self.M_UP and self.head_orientation != self.M_DOWN:
			self.move_modif = -1 * self.arena_size
			self.head_orientation = self.M_UP
		elif new_modif == self.M_RIGHT and self.head_orientation != self.M_LEFT:
			self.move_modif = 1
			self.head_orientation = self.M_RIGHT
		elif new_modif == self.M_DOWN and self.head_orientation != self.M_UP:
